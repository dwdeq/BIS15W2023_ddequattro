...
```
```{r}
mammals <- replace(mammals, mammals < 0, NA) 
```
...
```{r}
mean(mammals[,11])
colMeans(mammals[,11],na.rm=TRUE)
```
...
```{r}
col_names <- c("order","family","genus","species","mass","gestation","newborn","weaning","weanMass", "afr","maxLife","litterSize","littersPerYear")
colnames(mammals) <- col_names
names(mammals)
```
