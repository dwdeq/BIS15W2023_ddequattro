...
2. We can use operators such as <, >, ==, <=, >=. Show all values in `my_vector_sequence` that are less than 10.  How about less than or equal to 10?  
```{r}
...
```{r}
totalSum <- colSums(all_harry_potter_matrix)
totalSum[3]
```

2. Add this information to the data matrix. Hint: you are adding a row, not a column.  
```{r}
all_hp_matrix <- rbind(all_harry_potter_matrix,totalSum)
all_hp_matrix
```
