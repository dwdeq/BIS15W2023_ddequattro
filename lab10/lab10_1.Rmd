...
life_history %>%
  group_by(order) %>% 
  summarize(log10_mass=log10(mean(mass, na.rm=T))) %>% 
  ggplot(aes(x=order,y=log10_mass))+geom_col()+coord_flip()
```
...
```{r}
life_history %>% 
  ggplot(aes(x=gestation, y=wean_mass))+geom_point()+geom_smooth(method=lm,se=F)+
  scale_y_log10()+
  scale_x_log10()
```
...
```{r}
elephants %>% 
  ggplot(aes(x=age,y=height)) + geom_point() + geom_smooth(method=lm,se=F) + scale_x_sqrt()
```
...
elephants %>% 
  ggplot(aes(x=age,y=height)) + geom_point() + geom_smooth(method=lm,se=F) + scale_x_sqrt() +
  labs(title = "Elephant Age (adj.) vs. Height",
       x = "sqrt Age",
       y = "Height")
```
...
homerange %>% 
  ...
  labs(title = "Mass vs. Homerange (adj.)",
        x = "log10 Mass (kg)",
        y = "log10 Homerange (km)") +
  theme(plot.title = element_text(hjust = .5))
```
