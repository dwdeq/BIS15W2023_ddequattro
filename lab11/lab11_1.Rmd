...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="rosybrown1", alpha  =0.3 , color = "red4")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(aes(y = ..density..), fill = "pink3", alpha = 0.4, color = "black")+
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = ..density..), fill = "lemonchiffon", alpha = 0.5, color = "black")+
  geom_density(fill="pink2", alpha  =0.5 , color = "black")+
  labs(title = "Distribution of Homerange")
```
...
```{r}
homerange %>% 
  mutate(hra_category = case_when(log10.hra <= 3.65 ~ "very small",
...
homerange %>% 
    mutate(hra_category = case_when(log10.hra <= 3.65 ~ "very small",
                                 ...
                                 log10.hra > 4.59 & log10.hra <= 6.02 ~ "medium",
                                 log10.hra > 6.02 ~ "large")) %>% 
  ggplot(aes(x = hra_category, fill = taxon)) +
  geom_bar()
```
