...
```
## # A tibble: 5 × 2
##   migratory_strategy average_body_mass
##   <chr>                          <dbl>
## 1 Long                            742.
## 2 Moderate                       1618.
## 3 Resident                      10715.
...
## 5 Withdrawal                      831.
```
#Ducks with a farther migratory ranges are more likely to be lighter than ducks with short ranges. 
...
```r
#Which species (based on mean eggs per clutch and the age of sexual maturity) are more r-selected than average? Which are more K-selected?
ecosphere %>% 
  ##I'm limiting variables to their names and the relevant data
  select(common_name,scientific_name,mean_eggs_per_clutch,mean_age_at_sexual_maturity) %>% 
  #Now I'm creating a ratio for each species; the ratio is measured by comparing the "mean eggs per clutch" and "mean age at sexual maturity" with their averages across every other species.
  ##Basically, I'm measuring if the species has above or below average eggs per clutch and age of sexual maturity, and then comparing them to see how they compare.
  mutate(rk_ratio = (mean_eggs_per_clutch/mean(mean_eggs_per_clutch))/(mean_age_at_sexual_maturity/mean(mean_age_at_sexual_maturity))) %>% 
  #Lastly, based on the ratio, I'm categorizing each species as "more r-selected" or "more K-selected"
  mutate(selection_type = ifelse(rk_ratio > 1, "more r-selected", "more K-selected")) %>% 
  arrange(desc(rk_ratio))
```

```
## # A tibble: 551 × 6
##    common_name         scientific_name          mean_e…¹ mean_…² rk_ra…³ selec…⁴
##    <chr>               <chr>                       <dbl>   <dbl>   <dbl> <chr>  
##  1 Ruffed Grouse       Bonasa umbellus              10.5     0.3   12.3  more r…
##  2 Green-winged Teal   Anas crecca                   8       0.4    7.03 more r…
##  3 Sharp-tailed Grouse Tympanuchus phasianellus     11       0.6    6.45 more r…
##  4 Northern Bobwhite   Colinus virginianus          17       1      5.98 more r…
##  5 Blue-winged Teal    Anas discors                 10       0.6    5.86 more r…
##  6 Northern Shoveler   Anas clypeata                10       0.6    5.86 more r…
##  7 California Quail    Callipepla californica       13       0.8    5.72 more r…
##  8 Wild Turkey         Meleagris gallopavo          12.5     0.8    5.50 more r…
##  9 Scaled Quail        Callipepla squamata          15.5     1      5.45 more r…
## 10 Chukar              Alectoris chukar             15.5     1      5.45 more r…
## # … with 541 more rows, and abbreviated variable names ¹​mean_eggs_per_clutch,
## #   ²​mean_age_at_sexual_maturity, ³​rk_ratio, ⁴​selection_type
```

Please provide the names of the students you have worked with with during the exam:

```r
contributors <- c("Sidney Rang","Ayana Carpenter")
```
