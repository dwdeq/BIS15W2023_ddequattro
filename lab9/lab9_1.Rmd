...
```{r}
homerange %>% ggplot(mapping = aes(x=log10.mass, y=log10.hra))+geom_point()
```
...
```{r}
homerange %>% ggplot(mapping = aes(x=log10.mass, y=log10.hra))+geom_point()+geom_jitter()
```

To add a regression (best of fit) line, we just add another layer.
```{r}
homerange %>% ggplot(mapping = aes(x=log10.mass, y=log10.hra))+geom_point()+geom_smooth()
```

### Practice
1. What is the relationship between log10.hra and log10.preymass? What do you notice about how ggplot treats NA's?
```{r}
homerange %>% ggplot(mapping = aes(y=log10.preymass, x=log10.hra))+geom_point() + geom_smooth(method = lm, se = F, na.rm = T)
```
...
homerange %>% 
  arrange(log10.mass) %>% 
  head(log10.mass, n=10) %>% 
  ggplot(mapping = aes(x = common.name, y = log10.mass)) + 
  geom_col() +
  coord_flip()
```
