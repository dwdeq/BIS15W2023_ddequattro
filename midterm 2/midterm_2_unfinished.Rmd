...
```{r}
surgery %>%
  filter(ahrq_ccs == "Colorectal resection" |
         ahrq_ccs == "Small bowel resection" |
         ahrq_ccs == "Gastrectomy; partial and total" |
         ahrq_ccs == "Endoscopy and endoscopic biopsy of the urinary tract" |
         ahrq_ccs == "Spinal fusion") %>%
  group_by(ahrq_ccs) %>% 
  summarize(mortality = mean(ccsmort30rate)) %>%
  ...
  scale_fill_brewer(palette = "RdPu") +
  theme_classic() +
  labs(title = "30 Day Mortality Rates of the 5 Deadliest Surgeries",
       x = "Body Mass Index",
       y = NULL) +
  theme(plot.title = element_text(hjust = .5,
                                  size = 10))
