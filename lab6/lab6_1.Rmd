...
msleep24 <- msleep %>%
  ...
  filter(!is.na(vore) & sleep_total_24 < .3) %>% 
...
```{r}
#summarize treats NAs the way most functions do; if there are any NAs present for a group, it will output NA as well
```
...
every value is NA; by removing NAs, there is no data left to manipulate
