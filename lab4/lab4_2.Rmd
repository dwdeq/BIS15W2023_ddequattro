...
```
```{r}
mammals <- replace(mammals, mammals == -999, NA) 
fish <- replace(fish, fish == -999, NA) 
```
...
```{r}
filter(select(mammals,family:gestation),family == "Bovidae" & mass > 450000)
```
...
```{r}
q6_dframe <- filter(select(mammals,"genus",mass:newborn), genus == "Lophocebus" | genus == "Erythrocebus" | genus == "Macaca")
q6_dframe
```

```{r}
colMeans(filter(q6_dframe, genus == "Macaca")[,4])
```
546.50 > 449.10 > 462.50; therefore, Lophocebus has the lowest newborn mass.
